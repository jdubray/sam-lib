 ...
 */
export class SamShapeError extends Error {
  /**
  ...
  constructor(key, violation) {
    super(`Invalid model write: ${violation}`)
 ...
 */
export class SamFrameError extends Error {
  /**
 ...
 */
export class SamValidationError extends Error {
  /**
...
const typeMatches = (specType, actualType) => (Array.isArray(specType)
  ? specType.includes(actualType)
  : specType === actualType)
...
export const validateProposal = (schema = {}, proposal = {}) => {
  ...
  Object.keys(schema).forEach((field) => {
    ...
    if (spec.type && !typeMatches(spec.type, typeOf(value))) {
      violations.push(`field '${field}' expected type '${displayType(spec.type)}', got '${typeOf(value)}'`)
    }
  })
  return violations
}
 ...
 */
export const checkShapeWrite = (shape, key, value) => {
  const spec = shape[key]
  if (spec === undefined) {
    return `undeclared model key '${key}' (declare it in modelShape, or mark it internal)`
  }
  if (value === null || value === undefined) {
    return spec.nullable ? null : `key '${key}' set to ${value} but not declared nullable`
  }
  if (spec.type && !typeMatches(spec.type, typeOf(value))) {
    return `key '${key}' expected type '${displayType(spec.type)}', got '${typeOf(value)}'`
  }
  return null
}
