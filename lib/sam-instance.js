...
import {
  enforceProposalSchema, checkShapeWrite, SamShapeError, SamValidationError, SamFrameError
} from './sam-strict'
...
export default function (options = {}) {
  ...
  const frozenModel = new Proxy(model, {
    ...
    set(target, prop, value) {
      if (typeof prop === 'string' && prop.indexOf('__') !== 0) {
        throw new SamShapeError(prop, `cannot write '${prop}' on the frozen pre-state; assign next.${prop} instead (#25)`)
      }
      target[prop] = value
      return true
    },
    deleteProperty(target, prop) {
      if (typeof prop === 'string' && prop.indexOf('__') !== 0) {
        throw new SamShapeError(prop, `cannot delete '${prop}' on the frozen pre-state; assign next.${prop} instead (#25)`)
      }
      delete target[prop]
      return true
    }
    ...
    set(target, prop, value) {
      if (typeof prop === 'string' && prop.indexOf('__') !== 0) {
        ...
        if (origin !== undefined && origin !== id) {
          throw new SamFrameError(`double-prime: variable '${prop}' assigned by more than one acceptor in a single step`, { doublePrime: prop })
        }
        stepAssignOrigin.set(prop, id)
        if (stepPrimed.has(prop)) {
          stepPrimed.get(prop).to = value
        } else {
          stepPrimed.set(prop, { from: model[prop], to: value })
        }
  ...
  const makeUnchanged = (id) => (...names) => {
    ...
    names.forEach((name) => {
      if (name !== '*' && modelShape && modelShape[name] === undefined) {
        throw new SamFrameError(`unchanged('${name}') does not match any declared modelShape variable (#25)`, { unknown: name })
      }
  ...
  const makeReject = (id) => (reason) => {
    if (nextStateMode) {
      ...
      if (written.length > 0) {
        throw new SamFrameError(
          `reject() after next-state writes: reject declares a NO-OP and would discard ${written.map((k) => `'${k}'`).join(', ')} — a declined branch must reject before writing; a successful step must not reject (there is no accept-with-reason primitive)`,
          { rejectAfterWrite: written, intent: currentIntentName }
        )
      }
    }
    stepRejections.push({ intent: currentIntentName, reason })
  ...
  const registerAcceptorMeta = (id, key) => {
    ...
    acceptorApiById.set(id, { next: makeNextProxy(id), unchanged: makeUnchanged(id), reject: makeReject(id) })
    if (!acceptorMeta[key]) {
      acceptorMeta[key] = { primes: new Set(), unchanged: new Set(), unchangedAll: false }
    }
  }
  ...
  const asAcceptor = (id, fn) => (proposal, api) => {
    ...
    return fn(proposal, { ...api, ...acceptorApiById.get(id) })
  }
  ...
  const commitNextState = (proposal) => {
    ...
    const missing = obligation.filter((key) => !assigned.includes(key)
      && !stepUnchanged.has(key) && !explicitAll)
    if (missing.length > 0) {
      ...
      if (assigned.length === 0 && stepUnchanged.size === 0) {
        throw new SamFrameError(
          `next-state frame empty for intent '${currentIntentName}': no acceptor assigned next.<var> or declared unchanged(...) — unhandled intent or missing frame (#25)`,
          { unaccounted: missing, unhandledIntent: currentIntentName }
        )
      }
      throw new SamFrameError(
        `next-state frame incomplete: variable(s) ${missing.map((k) => `'${k}'`).join(', ')} neither assigned in next nor declared unchanged (#25)`,
        { unaccounted: missing }
      )
    }
    stepUnchangedResolved = obligation.filter((key) => !assigned.includes(key)
      && (stepUnchanged.has(key) || explicitAll))
    // apply through sealedModel so stepWrites/stepMutations record the commit
    assigned.forEach((key) => { sealedModel[key] = nextDraft[key] })
  }
  ...
  const lastStep = () => {
    ...
    const primed = {}
    stepPrimed.forEach((value, key) => { primed[key] = value })
    return {
  ...
  let present = synchronize ? async (proposal, resolve) => {
    ...
    if (checkForOutOfOrder(proposal)) {
      ...
      if (acceptResults.some((result) => result && typeof result.then === 'function')) {
        if (nextStateMode) {
          throw new SamFrameError(
            `async acceptor on a non-synchronized instance: next-state writes after an await are discarded — pass createInstance({ synchronize: true }) or make the acceptor synchronous (intent '${currentIntentName}')`,
            { asyncAcceptor: true, intent: currentIntentName }
          )
        }
        if (devWarnings) {
          console.warn(`SAM: acceptor returned a promise on a non-synchronized instance (intent '${currentIntentName}') — writes after an await land outside the step; use synchronize: true for async acceptors`)
        }
      }
  ...
  const addComponent = (component = {}) => {
    ...
    const acceptorOperand = component.localState ?? (nextStateMode ? frozenModel : sealedModel)
