...
import { enforceProposalSchema, checkShapeWrite, SamShapeError } from './sam-strict'
...
export default function (options = {}) {
  ...
  const lastStep = () => {
    ...
    return {
      intent: currentIntentName, mutations, writes, rejections, classification
    }
  }
  ...
  const addComponent = (component = {}) => {
    ...
    if (namedActions) {
      namedActions.forEach((action) => {
        intentRegistry[action.__actionName] = { schema: action.__schema, domain: action.__domain }
      })
    }
    ...
    if (hasAsyncActions) {
      ...
      intentList = actionList?.map(action => (...args) => {
        try {
          if (isAllowed(action)) {
